...
//до кожного з яких школяр повинен ввести відповідь. Вказати правильність кожної відповіді – у випадку помилки назвати 
//вірний варіант
#include"stdafx.h"
#include<iomanip>
#include<iostream>
...
{
	string que;   //рядок для запитання
	string ans[4];        //для 4варіантів відповідей
	int right; //для вірної відповіді
};
...
for(int i=0;i<n;i++)   //вводимо запитання, формуємо тест
{ 
...
for(i=0; i<5; i++){
	...
	cout<<"\n"<<i+1<<". "<<x[t].que<<endl;
	for(int j=0; j<4; j++){
		cout<<"\t"<<j+1<<")"<<x[t].ans[j];   //зберігаємо відповідь користувача
	}
	cout<<"\nYour answer is: ";
	cin>>a;
